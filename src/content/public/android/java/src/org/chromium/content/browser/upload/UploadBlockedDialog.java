...
/**
 * Dialog for showing upload blocked notifications, using a custom snackbar.
 */
public class UploadBlockedDialog {
    ...
    /**
     * Called to show an "Upload blocked" snackbar.
     * 
     ...
     */
    public void show(Context context, String message, Callback<Boolean> callback) {
        // Add debugging log
        System.out.println("UploadBlockedDialog: show called with message: " + message);

        // Record that the dialog was shown
        recordUploadBlockedDialogEvent(UploadBlockedDialogEvent.UPLOAD_BLOCKED_DIALOG_SHOW);

        // Create and show the snackbar
        showUploadBlockedSnackbar(context, message, callback);
    }

    /**
     * Shows a custom snackbar for upload blocking at the top of the screen.
     */
    private void showUploadBlockedSnackbar(Context context, String message, Callback<Boolean> callback) {
        ...
        View customSnackbarView = createCustomUploadBlockedSnackbarView(context, message, callback);
        ...
        if (rootView instanceof ViewGroup) {
            ...
            new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(() -> {
                ...
                recordUploadBlockedDialogEvent(UploadBlockedDialogEvent.UPLOAD_BLOCKED_DIALOG_DISMISS);
                if (callback != null) {
                    callback.onResult(false);
                }
            }, 4000); // 4 seconds
     ...
     */
    private View createCustomUploadBlockedSnackbarView(Context context, String message, Callback<Boolean> callback) {
